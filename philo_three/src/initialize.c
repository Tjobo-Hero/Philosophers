...
/*   Updated: 2021/04/21 09:55:31 by timvancitte   ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
int	initialize_semaphore(t_data *philo_s, int i)
{
	...
	philo_s->sem_state = sem_open(SEM_STATE, O_CREAT, 0600, 0);
	if (philo_s->total_forks == SEM_FAILED
		|| philo_s->sem_eat == SEM_FAILED
		|| philo_s->sem_write == SEM_FAILED
		|| philo_s->sem_state == SEM_FAILED)
		return (SEM_ERROR);
	while (i < philo_s->total_p)
	{
		...
		philo_s[i].sem_state = philo_s->sem_state;
		i++;
	}
	return (0);
}
